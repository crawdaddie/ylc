...
int reinit_lang_ctx(Context *ctx) {
  ...
  //     LLVMTypeRef type_ref =  sym.data.TYPE_GLOBAL_VARIABLE.llvm_type;
  //     LLVMValueRef new_var = LLVMAddGlobal(ctx->module, type_ref, name);
  //     LLVMValueRef actual_val = LLVMBuildLoad2(ctx->builder, type_ref, global, "");
  //     LLVMBuildStore(ctx->builder,  actual_val, new_var);
  //   } 
  //   global = LLVMGetNextGlobal(global);
  // }
